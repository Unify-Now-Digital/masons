...
Deno.serve(async (req: Request): Promise<Response> => {
  ...
  const subject = (body.subject ?? conversation.subject ?? '(no subject)').trim();
  ...
  // Send as new thread (no threadId) so Gmail creates a new conversation
