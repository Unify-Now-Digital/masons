...
**Supporting checks.** `supabase/config.toml` has no `inbox-ai-thread-summary` entry (7 `verify_jwt=false` functions, all webhooks/OAuth) → `inbox-ai-rank` deploys **plain** (no flag, no config.toml entry) and every internal-key call must also carry `Authorization: Bearer <anon key>` to pass the gateway. (Corrected 2026-09-19: NOT like the summary function. Its missing config.toml entry did not mean verification was on; it was deployed with `--no-verify-jwt`, its verify_jwt is OFF, and it has been pinned false since 8cb0d5d. Rank is ON. Both were confirmed in the Dashboard. See docs/handoff.md T30.) tsc baseline holds **0 items** in `PageShell.tsx` and `UnifiedInboxPage.tsx`. `schema_migrations` appears in no tracked migration — recording it is a manual apply step. Baseline per-file grep before each edit remains mandatory (line-shift trap).
