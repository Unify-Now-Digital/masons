...
- **T-N2 (FR-015)**: most `border` classes on the five surfaces are functional control chrome (buttons :219/:227, checkbox, select, search input) and must NOT be removed. The actual "dividing lines": PersonOrdersPanel:260 header `border-b` (dies with the tab bar in C1 anyway), CustomerThreadList `divide-y divide-gardens-bdr` row separators, and 5 card borders across InboxContactTab (2), InboxFinancesTab (2), InboxHistoryTab (1). FR-015 is an enumerated-surface pass, not a sweep. **Corrected at T502 (2026-09-03), two errors in the enumeration above:** (i) the card surfaces in the tab components are **4**, not 5 — InboxContactTab's "second border" is the Edit-contact button (:85), functional control chrome by this note's own rule, and stays; the four are InboxContactTab:72, InboxFinancesTab:62 and :94, InboxHistoryTab:49. (ii) the note walked only the three tab components, so it missed **InboxOrderSummaryCard:49**, which carries the identical card class literal and is the Orders card's own surface — treating the four and skipping it would have left Orders as the column's only bordered surface (ruled IN at T004). Also: PersonOrdersPanel's header `border-b` did **not** die with the tab bar as predicted — C1 kept the row for the collapse button (now :264) and its rule with it; dropped at T502 instead (ruled).
...
- [x] T004 [US4] (G) Rule at C4 approval: FR-015 specific class choices over the T-N2 enumerated surfaces. **Ruled 2026-09-03**: (1) pill retoken via Tailwind `gardens-*` classes, not the inline-`style` form of the PipelinePage/InvoiceWorkspace precedents — keeping the `className` override prop live outweighs literal parity; (2) `InboxOrderSummaryCard:49` **IN** (T-N2 correction); (3) `PersonOrdersPanel` header `border-b` **dropped**; (4) card headers minimal (colour + weight + hover tint) — a heavier band adds chrome in a cycle whose complaint was too much of it.
...
- [x] T501 [P] [US4] (CC) Draft `InboxFilterPill.tsx` retoken: selected `bg-[#243D2E] text-white` (:24) → `background: var(--g-acc-lt)` + `1px solid var(--g-acc)` border + text `var(--g-acc-dk)` (unselected text `var(--g-tx)`), per the Finance chip pattern (InvoiceWorkspace.tsx:650/:671); add `aria-pressed={selected}`. Note in the diff: restyle reaches the flat view's `InboxFilterPillRow` (InboxConversationList:281) by design (T-2 ruling — shared component, no flat-specific work, no prop).
- [x] T502 [US4] (CC) Draft the contrast/divider pass over the T-N2 enumerated surfaces ONLY: CustomerThreadList `divide-y` row separators, the tab-component card borders, plus contrast on the rebuilt card headers (C1 surfaces). Functional control chrome untouched. gardens-*/`--g-*` tokens only, no new raw hex. T004 ruling at approval. **Applied set (per the corrected T-N2 + T004)**: `divide-y divide-gardens-bdr` → `space-y-1` (CustomerThreadList:188); `border border-gardens-bdr bg-white/90` → `bg-gardens-surf2` on InboxContactTab:72, InboxFinancesTab:62, InboxHistoryTab:49, InboxOrderSummaryCard:49; Finance grand-total strip `border … bg-gardens-page/80` → `bg-gardens-page` (:94); `CARD_TRIGGER_CLASSES` txs/medium → tx/semibold + `rounded-md hover:bg-gardens-bdr/40`; PersonOrdersPanel:264 header `border-b border-gardens-bdr` dropped. **C5c note (2026-09-03)**: two edits in this applied set no longer exist in the tree — `InboxFinancesTab:62` (card surface → `bg-gardens-surf2`) and `:94` (grand-total strip → `bg-gardens-page`) went with `InboxFinancesTab.tsx` when C5c deleted the Finance card. The C4 commit is unchanged history; the surviving surfaces are `CustomerThreadList:188`, `InboxContactTab:72`, `InboxHistoryTab:49`, `InboxOrderSummaryCard:49`, `CARD_TRIGGER_CLASSES` and the dropped `PersonOrdersPanel` header rule. T-N2 (:21) carries the same two dead anchors in its four-card-surface enumeration.
