...
7. **Given** `unreadOnly` is active, **When** the Customers or `Hidden (n)` pill is selected, **Then** the list shows unread **within** that filter — unread narrows the active pill rather than replacing it (C3c fix).
...
- **SC-004**: Customers-view header control count drops from 8 (audit D1 inventory) to ≤ 4 groups: icon cluster (Unread toggle, Mark unread, "+", collapse), pill row + channel control (**one row** — the FR-012 placement ruling), search. Group count unchanged by C3c; the cluster gains a fourth member and the header stacks 3 rows, not 4. Bulk-selection UI count: 0.
