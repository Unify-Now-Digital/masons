...
3. **Given** the filter row, **When** it renders, **Then** Awaiting and Unlinked pills are absent; All, Customers, Unread remain; `Hidden (n)` still appears when mutedCount > 0 and the row-level Unmute (:387–401) still works under it.
...
6. **Given** the icon-only Unread toggle beside "+", **When** clicked, **Then** it toggles the same `listFilter === 'unread'` state as the Unread pill — the two surfaces stay in sync, both with `aria-pressed`.
...
- **Order created while Orders card is collapsed**: today's flash + `scrollIntoView` (summaryRef :166, :355–360) targets a `display:none` subtree and would be invisible — see FR-008.
...
- **FR-008 (flash visibility)**: The order-created flash + scroll (:355–360) must remain user-visible: if the Orders card is collapsed when the flash triggers, it expands first (then scrollIntoView within the column). Assumption A-4.
...
- **A-3**: ~~Assumption~~ RULED (R-004): the "All" pill stays; the ruled removals are exactly Awaiting and Unlinked.
- **A-4**: FR-008's auto-expand of the Orders card on order creation is the intended flash behavior (alternative: drop the flash; it exists to confirm creation).
...
- **R-004 — RULED**: "All" pill stays. With Awaiting and Unlinked gone, All/Customers/Unread is already minimal and All is the reset. (FR-011.)
...
- **T-3 (open for plan time)**: If `BulkDeleteConversationsDialog` or `markedReadIds` turn out to be consumed by the flat view, they stay (FR-010's carve-out); only customers-view wiring goes. To be confirmed with grep evidence at plan time.
