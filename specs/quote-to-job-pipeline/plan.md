...
| `PersonOrdersPanel.tsx:82` → `orders.api.ts` `fetchOrdersByJobId` (job-orders path, found T023 20 Aug) | Archived orders excluded from the Order context panel; "Create invoice" button NOT summoned by archived quote orders (it preloads `uninvoicedJobOrders` → live Stripe invoice hazard); "New order" unaffected. Sole consumer of the fetch; filtered 20 Aug (decision a) |
...
4. (Wednesday agenda, beside F3 — from the T023 finding) Quote-history view as deliberate
   design: should archived quotes be shown view-only in the Order context panel (option c)?
   Currently filtered out entirely (decision a, 20 Aug).
