...
Branch: feature/restricted-role (3e9ff9c), cut from staging 6e9bd48. **Third org role `staff` C1→C7b COMPLETE, pushed, gated, browser-verified, NOT merged to staging — T31 below.** 6e9bd48's history already contains 26eacde and e709014 (session-start log), so the sidebar-order-form "NOT merged" wording that follows predates that merge. Prior: feature/sidebar-order-form (26eacde), cut from staging 14b4017. **feature/sidebar-order-form C1→C4 COMPLETE, pushed, browser-verified, NOT merged to staging — T30 below.** Staging since T29: f02b4d6 (T29 docs), 14b4017 (F-035 fix). **feature/order-timeline-progress C1–C3 COMPLETE (spec 50fcc41, C1 4887035, C2 3ac02d7, C3 84df88f), ff-merged to staging at 8978861 — T28 below. fix/email-frame-oscillation merged and deployed, staging HEAD 191e9ee — T27. ai-inbox-prioritisation C5 3eacaf4 + handoff 141995c MERGED to staging 2026-09-10 — T24.** Prior: feature/ai-inbox-prioritisation MERGED to staging 46e948e 2026-09-10 (C1–C4b), deployed; reviewer pass SKIPPED (per-edit review in chat throughout) (C1 16c7528 + apply f57089d, C2 2b0eeb8, C2b e359e5c, C2 record 5390eac, C3 6097aac, C3a 04944f0, C3b 215d2b5, C3c de28d33, C4 ac850d6, C4b 46e948e; T21/T22/T23/T24 below). Prior history on staging: feature/finance-consolidation MERGED 2026-09-02, C6 docs committed; reviewer pass on the full branch diff: SKIPPED. Blocks 1 (P0 invoice) and 2 (finance consolidation) closed. Block 3 search cycle CLOSED — feature/full-name-search C1a→C4 complete (C3 5719254; C4 docs = this commit; T13 below); MERGED to staging b425269 (fast-forward), gate green (tsc 54/54, lint 8/19, 11 tests), pushed 2026-09-03; branch may be deleted. Block 3 shell cycle COMPLETE on the branch — feature/inbox-shell-rebuild: C1→C4 + Phase 6 docs (e7b0ff6, T14 below), C5a daf4149, C5b df84dd0, C5c 51ecb0f (T15 below). **C6 6fa978e and C7 9550c73 both COMMITTED and PUSHED** (T16 = C6's record, T17 below = C6+C7 close-out) — C6 was areas 1+2 of three, area 3 (Actions dropdown + status pills) split to C7 and now shipped. C8 46141c5 (sidebar decoration removed, narrowed to 192px, F-031) closed the cycle; since then on the same branch: 0b28199 (F-032/F-033 findings + two backlog lines), 06d5a1b (internal-note button hidden; T18 below), 6f5169c (C9), dcfcc1d (C10), 0a93988 (docs) and 3087c10 (C10b). **Block 3 shell cycle CLOSED — feature/inbox-shell-rebuild MERGED to staging 2026-09-03 at 6edec8c** (fast-forward, 27 commits), C11 render-time quote stripping shipped in it (T20 below); gate GREEN on HEAD (tsc 54/54 item-diff 0 new, lint 8/19, build clean, 11 tests); branch may be deleted. **Scope warning for anyone picking this up: the branch absorbed FOUR features beyond its spec — C5 controls, C8 sidebar, and C9/C10/C10b/C11 message rendering. `specs/inbox-shell-rebuild/spec.md` describes C1–C4 only. Reading spec.md alone will not tell you what shipped; the T14–T20 blocks below are the record.** Next: migration drift audit, now OVERDUE and not queued (two confirmed instances, F-026 + F-033). Prior gate state, resolved: RED since C9 (lint 8/**20** against baseline 8/19) with C10 dcfcc1d, C10b 3087c10 and the docs commits landing over it; fixed in 494fa26, not a baseline raise. Prior: staging at chore/tooling-bootstrap (merged 2026-08-30); per-session tripwire history lives in the blocks below.
...
T31 (2026-09-21, third org role `staff`, Arin's ask: a colleague who creates invoices without seeing the financial dashboard; branch `feature/restricted-role` cut from staging 6e9bd48, HEAD 3e9ff9c; every commit gated, pushed and browser-verified; NOT merged): inputs were the two read-only reports `~/.claude/plans/read-only-investigation-deliverable-a-peppy-kite.md` and `read-only-investigation-plan-mode-modular-crystal.md`; the Phase 0 plan with scored predictions is `~/.claude/plans/implement-third-org-role-soft-thunder.md`. **What shipped: `staff` = Member's restrictions (every existing gate is "is admin", so a non-admin value inherits them) plus hidden aggregate-money UI. UI-LEVEL ONLY, not a data-access boundary — F-053.**
Commits: **C1 97eecc4** db: role CHECK widened to admin|member|staff + `change_member_role` accepts staff. **C2 53cbe75** role plumbing: `OrganizationRole` union, `canViewFinancials` (+ unit test), role select. **C3 1d96561** aggregate money UI hidden for staff; reporting and payments routes guarded. **C4 73167fa** create organisation is admin-only in Settings (UI-only). **C5 9f02a72** footnote role legend in the members panel (superseded by C7). **C6 6e0effc** settings tab order, Account default, Organisation tab admin-only. **C7 7291fb6** role help ("?" tooltips) in the role picker. **C7b 3e9ff9c** footnote legend removed.
Rulings (Giorgi, settled before Phase 0). **R1** role value `staff`; the Sidebar label logic is unchanged and renders "Staff". **R2** one pure function `canViewFinancials(role)` (`src/shared/lib/canViewFinancials.ts`), true only for admin|member; staff, unknown strings, null and undefined fail closed; exposed from `OrganizationContext` beside `isOrgAdmin`; no component compares against the literal `'staff'` (grep at C3: the union, the select item value, the unit test). **R3** hidden when `!canViewFinancials`: the `FinancePage` stat strip (all five `StatItem`s), the chip hover totals, the `HubPage` money trio (Avg job value, Outstanding, Collected this month), the Reporting nav item; `reporting` and `payments` are route-guarded by `RequireFinancials` (`src/app/components/`), redirecting to `/dashboard`; per-record amounts and the `JobsMapPage` selection total stay; `useInvoicesList` keeps running for staff. **R4** `add_organization_member_by_email` unchanged, no signature change anywhere: add as member, then switch in the role select. **R5** `_shared/organizationMembership.ts` untouched, no edge function change, no deploy (its `OrgRole` type lags — backlog).
Hazard rulings from the plan. **H3** `setActiveOrganizationId` does not toggle `isLoading`, so `FinancePage` stays mounted across an org switch; a stat filter set while the strip was visible could outlive it, so the filter handed to the workspace is DERIVED to `'all'` when `!canViewFinancials && isStatFilter(...)` — no effect, no state reset. **H4** the chip total is zeroed in `FinancePage`'s `tiles` memo and `InvoiceWorkspace.tsx` is byte-untouched: its only use of `totalPence` is the chip `title`, already `undefined` at 0 (3 lines: prop type, destructure, title); the memo carries a RELIANCE comment saying so. **H5** the Hub "See payment reconciliation" button is hidden for staff (it was the only in-app link to a guarded route besides the nav). Also: `useFinanceTotals` / `useConfirmedOrdersStat` are skipped for staff INSIDE the hooks (`enabled: … && canViewFinancials`, no signature change); TanStack Query is v5 (5.59.16), where a disabled query is not `isLoading`, and nothing outside the strip reads either query. The guard redirects only on a POSITIVELY KNOWN role (`isLoading || role === null` → render nothing): `OrganizationContext` reports `isLoading` false for a moment on a hard load, before the auth session resolves (the `sessionUserId` effect runs once with null and clears the flag). `PageShell` withholds the `Outlet` and the `Sidebar` until an organisation is active, so neither the guard nor the nav filter renders inside that window — an admin hard-loading `/dashboard/reporting` lands on Reporting (browser-verified), and Reporting does not pop in late.
Follow-ups requested during the session. **C4**: the Settings create card gated with `isOrgAdmin` like the Delete card; `create_organization` stays callable by any authenticated user because first-org onboarding (`PageShell`'s Welcome screen, its own `CreateOrganizationModal`, the only other entry point) depends on it — server gate deferred (backlog). **C6**: the Settings tabs were uncontrolled `defaultValue="organisation"`; now controlled, order Account / Integrations / Organisation / Monitoring, default Account, Organisation admin-only like Monitoring, grid 4 columns for admins and 2 otherwise; the shown tab is derived (an admin-only tab reads as Account when the current role cannot see it, and comes back if the role does). No code navigates to Settings with a tab; the only redirect into Settings is `revolut-oauth-callback`, which now lands on Account (backlog). `gmail-oauth-callback` returns to `/dashboard/inbox`. **C7**: a local `RoleItem` in `OrganizationMemberRoleSelect` built on `SelectPrimitive.Item` — the shared `SelectItem` puts ALL children inside `ItemText`, which the closed trigger mirrors, so the "?" sits outside it; the icon stops pointerdown/pointerup/click so it cannot select a role; tooltip content is portalled locally (the shared `TooltipContent` is not, and would clip inside the select); `ui/select.tsx` and `ui/tooltip.tsx` untouched, no new dependency. Sole admin: every non-admin item is disabled (was `member` only). Accepted: disabled items show no tooltip; no hover on touch. **C5** (footnote legend) was superseded by C7 and removed in **C7b**.
DB apply: migration `20260921130000_org_role_staff.sql` applied by hand in the Dashboard 2026-09-21, four statements, read back (constraint def, `proconfig {search_path=""}`, `prosecdef` true, `proacl` postgres + authenticated + service_role). The constraint is swapped in ONE `alter table` (drop + add), so the table was never without a CHECK; its name exists only in the live catalog. The paste introduced CR at position 193 (the file is LF — confirmed by Giorgi); stripped server-side with the `supabase/CLAUDE.md` remedy, position 0 after, all four values re-read. The read-back is recorded in the migration's comment block. The draft timestamp `20260921120000` collided with the same day's permit_status migration; caught at the (j) read.
**Tripwire: 3/3 reached in Phase 0** — a2 (router wrapper shape: JSX `<Routes>`, `ProtectedRoute` > `PageShell`, not an object route around `Dashboard`), b3 (the loading window above), h2 (the `change_member_role` migration file already had `search_path = ''`; predicted `public`). **Giorgi override 2026-09-21** to finish the remaining mechanical reads (live `pg_proc` row, latest migration timestamp, tsc-baseline grep, vitest include). Zero further misses there or in C1–C3. The follow-up block (C6/C7 reads) ran 2 misses on its own count: the Settings tabs were uncontrolled, not URL-seeded; the Gmail OAuth return goes to the inbox, not Settings. Gate at each commit: tsc 53/53 item-diff 0 new and 0 shifted (no baseline key sits in any touched file), lint 8/19, 99 tests after C2 (+6 `canViewFinancials`). Transient tsc errors seen by the edit hook mid-file (interface before value; type after state; import removed before usages) were edit-order artefacts, each cleared by the next edit.
Process observation: during browser checks a test account was switched to Staff in the LIVE SM org and restored to Admin. Runtime role testing belongs in the E2E org.
Still open: merge to staging, production release, switch the colleague to Staff, message Arin. **Nobody is put on staff until C3 is on production** — a staff row against a build without C3 sees everything a Member sees.

T30 (2026-09-18/19, sidebar-order-form C1→C4: order form beside the conversation with AI prefill, Arin's ask; branch `feature/sidebar-order-form` cut from staging 14b4017, HEAD 26eacde; every commit pushed and browser-verified; NOT merged): spec 422baff, plan eb3c8de, tasks 6346e93, plus amendment commits. `specs/sidebar-order-form/tasks.md` is the record of the rulings and read results; this block does not restate them. Staging before the cut: f02b4d6 (T29 docs), then **14b4017, the F-035 fix**: membership guard on `inbox-ai-thread-summary` in all three scopes. findings.md and backlog.md called F-035 open until this commit.
**C1 0fcbd2d**: `CreateOrderDrawer` gains `presentation` (default 'modal'). `DrawerContent` gains a separate `side` branch; the default branch is textually unchanged. The inbox grid gets a 440px right track. Person-bound lifecycle: an open-time snapshot of person, job, email, phone and presentation; a Discard / Keep editing confirm when a dirty form sees a different person; `beforeunload` while the side form is dirty (R-001..R-003, R-011, R-012). **D-4**: the snapshot pins the S5 job at open, which fixes flat-view S5 orders getting `job_id` null or a deduped person's other job (finding). **D-5**: a generation key gives a fresh form per inbox open. A prop-driven close never reset the form in any host (finding); only the inbox is fixed (backlog). **C1a 052a5cc**: the side form opens from 1024px. The list stays visible at ≥1280px and collapses transiently below that; the stored preference is never written. Esc is ignored only when focus is in a text field outside the form (R-002, R-012 amended). **C1b 673a83b** (R-013, timeboxed, option A, all seven browser checks pass): `SideFocusRelease` mounts a non-trapping FocusScope one commit late. It sits on top of Radix's focus-scope stack and pauses the dialog's trap for the life of the form. `@radix-ui/react-focus-scope` is pinned exact to 1.1.0 (finding). **C2 f664f56**: side-host schema, superseded by **C2a eade740 + 3abc248**: `sku` and `location` optional in every order form, null-normalised writes, stars removed (R-004 amended). **R-014 770a9bc**: the Deceased Name star is removed; the schema never required the field.
**C3 79f6577**: `inbox-ai-extract-order` accepts a user JWT only; there is no internal-key path (R-008). Membership is checked before any table read. The org comes from the body, never from a row, and body ids are not referenced after the owned-id list. New `_shared/evidenceFilter.ts` (evidence substring guard, value caps, linked-person name guard) and `_shared/conversationTranscript.ts` (an import-free split, D-9, with `keepHead`), both covered by vitest. Review fixes **69c81de**: log the DB error code only, and a comment that a failed membership lookup reads as "denied". **T044 8cb0d5d**: `verify_jwt = false` pinned for `inbox-ai-thread-summary` to match its live setting. **T043**: the keepHead-0 golden test is the AC-008 guard. `inbox-ai-rank` was NOT redeployed, so its live bundle predates the D-9 split (backlog). **C4 26eacde**: `useOrderPrefill` makes one call per open and fetches the person's open conversations itself (D-16). `applyPrefill` fills empty, untouched fields only. AI marks show the evidence quote in a tooltip. Places suggests on focus (R-005, FR-016..FR-021, D-11..D-18). The C4 commit cites D-11..D-18, and D-13 and D-15..D-18 are annotated in code; tasks.md stops at D-10. Measured: `completion_tokens` 117–143 against the 1500 ceiling (D-10); prefill latency ~2.1–2.3 s.
Read results that were only in tasks.md. **T001 spike** (discarded): no backdrop, the conversation scrolls, copy/paste into the form works, Esc closes, and a mouse drag on the form body CLOSED the drawer (→ `handleOnly`). A composer click appeared to work in the spike, but the T002 source read predicted snap-back and the C1 browser check proved the source right: the trap was active and the next keystroke overwrote the last-focused field (→ R-013, C1b). T002: `<DrawerOverlay />` must stay in the side branch (finding). T003: a latent unmount through the `ghl` inbox-source ternary, unreachable while `SHOW_GHL_INBOX_TAB = false` (finding). Route navigation with a dirty form loses the draft silently (R-011, backlog). **D-1**: the list collapse is derived, never set. **D-2**: `onCloseOrder` left as is. **D-3**: the snapshot fills email/phone once if they were undefined at open. The C1 browser check did not exercise T7 (flat-view same-person switch); staff use the Customers view only. [OPT]: T016 was cut, so the side header does not name the bound person. Both T035 guards were taken: `sku` / `customer_name` must appear in their own evidence, and `customer_name` is dropped when its evidence is a `From:` line.
**T019 SKIPPED by ruling.** Verified directly instead by the `git show` check on `drawer.tsx`'s default branch, plus plan.md checklist 13 and 13'.
Tripwire for the whole cycle. Investigation (2026-09-18): 3/3 after Q2 (P1.3, P2.1, P2.2), then **override 1**: Giorgi reset the count to 0/3. 3/3 again after Q7 (P4.2, P6.2, P7.2), then **override 2**: Q8 was read after it. Phase 0 (2026-09-18): a stop was proposed at 3/3 in A4 (P4.1: the app nav is 192px, not the backlog's 220), then **override 3**: continue Phase 0 only, and plan.md was not written that session. Phase 0 closed at five misses (P1.3, P3.2, P4.1, P4.3 half, P5.3) plus a P5.4 count miss. The T001–T003 session stopped at 2/3 on the route-navigation stop condition. The T004–T006 session ended at 3/3: persistence is an effect, not a handler; Places is not a `setValue` site; `handleNewOrder` is async with two open sites. All of its reads were complete before the third miss. Both C3 read sessions ended at the tripwire. One C3 session's third miss was a CRLF file claimed as LF, with moved blocks claimed byte-identical (finding). tasks.md does not record the tallies for the implementation sessions. This docs session: 0/3.
Process: an Explore subagent ran live row counts during E2E seeding. That is outside the 2026-09-10 ruling, which makes business-row queries Giorgi's.
E2E seed fixtures `SEED-SOF-0918` (E2E org only). Their cleanup script is kept outside the repo, and cleanup has not been run.
Doc corrections in this commit: F-035 marked fixed in findings.md and struck in backlog.md. The claim that the summary function's gateway verifies is corrected in place in T22 below and at both sites in `specs/ai-inbox-prioritisation/plan.md` (supporting checks, C2 deploy note). It started in that plan's supporting checks, which read a missing config.toml entry as "verification on", and T22 copied it from there.
Next: F-042 (`inbox-ai-suggest-reply` membership check, its own commit); seed cleanup (Giorgi); the message to Arin (R-010).

T29 (2026-09-11/12, Arin branch reviews; docs only, no src/ edits, no git; staging HEAD 9723a35): four Arin branches reviewed via diff files (CC cannot run git) — plan files `~/.claude/plans/read-only-review-of-three-federated-dawn.md` (columns / timeline / deceased, tripwire 3/3 at close) and `read-only-review-of-review-inquiries-dif-bright-cocke.md` (inquiries, 0/3). **arin/orders-columns-persist** (from 8978861, clean): MERGED after fixes — merge b5cefb2. Reconciles the orders column registry drift recorded in backlog (3 dead ids out: `progress`, `depositDate`, `installationDate`; 4 live ids in: `customerType`, `photo`, `permitStatus`, `proofStatus`; registry = the renderer's 15 ids in renderer order, `orderColumnDefinitions.tsx:89-465`); sticky Actions column. Fix **1dbbe66**: `stopPropagation` on the customer popover trigger + `PopoverContent` — the new whole-row `onClick` double-fired the customer popover and the order sidebar; baseline `SortableOrdersTable.tsx(471,23)` RESOLVED by Arin's `isValidElement<{ style? }>` generic, re-anchored in the same commit → baseline count now **53**. **arin/orders-timeline-v1** (from c546e38, conflicts on 3 files): SUPERSEDED by feature/order-timeline-progress (T28); its "Infinity" fix deleted the sidebar days-until-due surface rather than guarding it; fold = one null-due guard (`OrderDetailsSidebar.tsx:179`, `:358-359`), pending (backlog). **arin/order-deceased-phase1-2** (from 8978861, clean): NOT MERGED — F-039 (migration RLS `to anon, authenticated using (true)`) and F-040 (name convention inverted vs the partner portal; equal-name rule blanks `customer_name`; unguarded dual-write); migration `20260910121500` NOT applied live (`to_regclass` null, `schema_migrations` 0 rows); no spec in the repo (Awaiting Arin). **arin/ux-inquiries-readability** (from c546e38, clean — AC-008 fence held: `UnifiedInboxPage.tsx` hunks at :10, :31, :1080, :1196 only, the C3a/C3c block untouched): MERGED after fixes — merge 47f6faf. **3da17f2** `isSelfHandleGroup` made unlinked-only — the `p:` branch hid the 23 linked SM people whose latest thread carries the org mailbox (F-036 count), reversing the C3b ruling in T23; the gmail status gate dropped (a revoked connection made every self-sync row reappear); test fixtures → example.com. **9723a35** enquiry labels for `appointment` / `call` / `shortlist` — portal `submit.js:1497` validates five channels and `inquiryRowLabel.ts` covered two. `enquiries` RLS confirmed live (`enquiries_select_by_org`, `enquiries_modify_by_org`, org-membership qual) with no tracked migration creating them — F-041. `tokens.css` darkens `--g-txs` / `--g-txm` shell-wide (consumed as `gardens-txs/txm`, 446 occurrences / 82 files; muted now passes AA). Gate after merge 53/53, lint 8/19, 40 tests. **Ruling 2026-09-12 (Arin): customer = paid = `people.is_customer`; invoiced-not-paid is not a customer** — the Orders Client badge (`jobs.stage ≥ invoiced`) is now out of line (backlog). Backlog +7 (null-due guard, selfHandles/needsAttention unification, badge alignment, unbounded enquiries select, cycling pill UX, invoices `dueDate` width drift, drift-audit scope); Awaiting Arin +2 (order_deceased one-page spec; pill UX).

T28 (2026-09-10, order-timeline-progress C1–C3, Arin P2 timeline bar; branch `feature/order-timeline-progress` off staging 191e9ee, COMMITTED, pending ff-merge): spec `specs/order-timeline-progress/spec.md` at **50fcc41**. **C1 4887035** — pure helper + 18 vitest cases. **C2 3ac02d7** — Timeline column on the Orders table: renderer + `tableViewPresets` registry entry, now passed into the row loop; tsc baseline re-anchor `SortableOrdersTable` (469,23)→(471,23) and, because the baseline file was staged whole, `OrderDetailsSidebar` (761,32)→(762,32) landed in C2, not C3. **C3 84df88f** — sidebar Timeline Progress rewired from deposit→install to the helper; shadcn `Progress` replaced by `PaymentProgressBar` with `TIMELINE_BAR_TONE`. Rulings R-1..R-5 (in the spec): start = `orders.deposit_date` (11/15 SM orders carry it; `jobs.paid_at` has NO code writer in either repo; portal payments key on `invoice_id`, not order); surfaces = Orders table + sidebar. Verified on localhost dev against SM: 11 bars / 4 "No deposit date"; over-plan red (ORD-000231, wk 13 of 12); non-default plan honoured (wk 5 of 16); sidebar = cell (ORD-000233, wk 8 of 12). Gate 54/54, lint 8/19, 29 tests. Tripwire 1/3 (the baseline-key delta; line-count slips excluded). The preceding investigation session ran to 7 structural misses; Giorgi override at 4 to finish Areas 3–4. **C4 (2026-09-10, spec amendment Story 4 / FR-008)** — Timeline cell on the Finance expanded-invoice order row: `ExpandedInvoiceOrders.tsx` gains a non-exported `OrderTimelineCell` in the row's pre-existing empty `<TableCell>` (was :158; 8-cell layout and the four `colSpan={8}` rows unchanged — the expanded row was never aligned to the dynamic invoice header), helper consumed via `@/modules/orders` (two new barrel exports, OQ6 precedent; the barrel was already in this file's graph through `CreateOrderDrawer.tsx:27`); no fetch change — `fetchOrdersByInvoice` is `select('*')` and `normalizeOrder` spreads. `InvoiceWorkspace.tsx` untouched (keys (123,29)/(812,31) stable). Backlog P2 line struck. **Tripwire 3/3 in the read-only phase**, all structural: hook location (orders/hooks, not invoicing), orders barrel exists, select is `*` not explicit; Giorgi override to apply, count carried at 3/3, stop on any further miss. Next: ff-merge to staging, deploy, Arin told once that `timeline_weeks` is a default 12 on all but one order (Awaiting Arin in backlog).

T27 (2026-09-10, email frame oscillation + auto-scroll; branch `fix/email-frame-oscillation` — 5a93b47, 388ae16 and f5a1da2 ff-merged from it, 191e9ee committed directly on staging; staging HEAD 191e9ee, deployed): staging-only "vibrating bubbles" on one SM thread. Not the build (vite preview on 8080 — the only localhost origin on the Supabase Auth allow-list; 4173 bounces to the production Site URL), not extensions (Incognito), width-dependent (reproduced locally by window width alone). Live logger: one frame alternating 372↔329px, scrollTop following. Fixes: **5a93b47** probe reads `iframe.clientWidth` (necessary, not sufficient); **f5a1da2** frame stylesheet `html{overflow-y:hidden}` — the loop was the MEASURE: the frame document grew its own vertical scrollbar, narrowed ~15px, text rewrapped two lines; verified locally (30 identical readings) then on staging after hard refresh — the first deploy still vibrated because C1b was uncommitted, caught by git log; **388ae16** `.order('id')` tiebreak on `fetchMessagesByConversation(Ids)` — correct, NOT the cause here (no `sent_at` ties on the thread); **191e9ee** auto-scroll: `pinnedToBottomRef` + parent-side ResizeObserver on a new list wrapper; flat view's dead `autoScrollResetKey` now honoured. F-037/F-038 filed at 5a93b47. Reviewer pass SKIPPED (per-edit review in chat).

T26 (2026-09-10, enquiry → order check; Churchill invoice memo check): (a) enquiry → order reproduced OK on staging (order created, hard-deleted after); Arin's report was the blocking modal, folded into the sidebar-form spec; two 2026-09-01 `sku='test'` SM orders found live (ids 11d20a2e…/459674d3…), disposition pending (backlog). (b) Memo check: the three Stripe write functions have no org branch; SM vs Churchill differ only by Stripe account template — logo, numbering prefix, address, and bank details in SM's FOOTER vs Churchill's DEFAULT MEMO. Trap: Mason writes invoice notes into `description`, which REPLACES the default memo, so a Churchill invoice with notes loses its bank details — bank details must move to the footer before any memo change. Order number appears only as the ORD- prefix on line items; the deposit is not represented on any invoice; Churchill has never issued a non-void invoice through Mason. Also: production mason.unifynow.digital serves a months-old pre-consolidation build (Awaiting Arin).

T24 (2026-09-10, ai-inbox-prioritisation C5: sweep-on-open + panel header and row meta; FR-014 / FR-019 amendments): **C5 `3eacaf4`** — TWO files, no page, host or context edit. Sweep-on-open needed no host change: `NeedsAttentionPanel` mounts fresh on every open because Radix unmounts `SheetContent`'s children when closed (`src/shared/components/ui/sheet.tsx:57-72` — `SheetPortal` = `SheetPrimitive.Portal` :14, no `forceMount` on Portal/Overlay/Content and none forwarded by `AiPanelHost.tsx:41-45`), so the panel simply calls `useInboxAiSweep({ k: 25 })`; k=25 is the function's K_MAX (`inbox-ai-rank/index.ts:21`, body param `k` at :38 and :366-368, clamped there rather than rejected). While the sheet is open a page re-render re-creates the panel element but React reconciles same-type-same-position, so the sweep does NOT refire mid-open. `useInboxAiSweep` now takes `{ k?: number }` and returns `{ run, isRunning }`, keeping the once-per-mount-per-org ref; `UnifiedInboxPage.tsx`'s bare call is untouched and ignores the return. Two additions beyond the brief, both approved at diff: an `inFlightRef` re-entrancy guard so a double-click cannot fire two sweeps (per hook INSTANCE — page and panel each hold their own, and an overlap is safe because the function claims rows before scoring, T22 [A6]); and `k` omitted from the request body entirely when undefined, so the page's request stays byte-identical to C3's. Panel header gains "Updated <age>" — max `ai_scored_at` over the UNSLICED scored set, so the label cannot change just because the page-size chip did: `selectNeedsAttention` → `selectNeedsAttentionAll` + a local slice, which also stops the full sort re-running on every chip click — plus a `RefreshCw` button aria-labelled "Refresh ranking", spinning and disabled while `isRunning`; both omitted when nothing is scored. Header row became `flex-wrap` so the third group wraps instead of squeezing the subtitle at `md:max-w-md`. Row meta moved to its OWN line under the name (the name no longer truncates against three siblings): `[category chip] [High/Medium band] [Us · 8h | Them · 36h]`. Category chip = capitalised `ai_category`, hidden for 'other', neutral `bg-gardens-page` / `text-gardens-txs`. Who-is-waiting reuses the EXPORTED `deriveBallInCourt` (`inboxBuckets.ts:136`) with the same wording as the customer list rows (`InboxAgingBadge.tsx:59,:61`) — **no new export was needed**: `computeAging` was deliberately not used (it needs the page-level `ClassificationContext`, `UnifiedInboxPage.tsx:513-514`, and its SLA colours would fight the band badge), and the age string comes from the panel's existing C3 copy of the module-private `formatShortAge`, split locally into `formatAgeMs(ms)` + `formatAge(iso)`. Gate after apply: tsc 54/54 item-diff 0 new, **0 baseline shifts** (structural, not hopeful — grep of `specs/inbox-sidebar-multi-tabs/tsc-baseline-items.txt` for `NeedsAttentionPanel`, `useInboxAiSweep`, `inboxBuckets`, `needsAttention` returns 0, so neither file holds a key that could line-shift), lint 8/19; Giorgi's run is the gate. Line deltas 61→106 and 197→282, both inside the predicted bands. **Tripwire 2/3** — two live-count prediction misses, both in the read-only phase: Churchill predicted 0 scored rows, actual 1 (its inbox mount swept, and only 1 of ~539 open rows was rankable — consistent with F-030's message-less corpus); SM inbound-timestamp coverage predicted >80%, actual 75%. Zero misses in the write phase. **Process ruling (Giorgi, 2026-09-10): CC ran an UNASKED live population read (scored counts, directional-timestamp coverage, category distribution) — outside the catalog-and-code line. Business-row queries via supabase-ro are Giorgi's to run, `count(*)` and aggregate-only included, even scoped to the two live orgs. No harm this time; not to be repeated unasked.** One load-bearing result from that read is retained because the code depends on it: every scored row in BOTH live orgs carries at least one directional timestamp, so `deriveBallInCourt` never returns null on today's data and the plain-age fallback is defensive only. Spec amendments **FR-014** (row shows category and who-is-waiting) and **FR-019** (sweep also runs on panel open, plus a manual refresh) shipped in code with `spec.md` deliberately NOT edited — this block is the record. Not done: browser verify on staging (Churchill is the cheap case — one scored row, category 'other', so the chip should be absent while the band badge and who-is-waiting still render), the `npm run gate` build/unit legs, reviewer pass, merge.

T23 (2026-09-10, ai-inbox-prioritisation C3 → C3c + C4 docs: the AI button host, the "Needs attention" panel and the mount sweep; plan C3/C3a + two post-verify follow-ups): **C3 6097aac** landed seven files — `src/shared/context/aiPanel.ts`, `src/shared/context/AiPanelContext.tsx`, `src/shared/components/AiPanelHost.tsx`, `PageShell.tsx` (+5), `src/modules/inbox/types/inbox.types.ts` (+11), `NeedsAttentionPanel.tsx`, `useInboxAiSweep.ts` — with six deviations from plan C3, all approved at diff: the context is split `.ts`/`.tsx` because keeping the hook and the provider in one `.tsx` trips react-refresh `only-export-components` and would have raised lint to 8/21 (same class as the C9 split in the shell cycle); the provider owns open state rather than the host, because the registering page needs `close()` after a select; no `SheetDescription` and `aria-describedby={undefined}` on the sheet content, with the subtitle carried inside the panel node instead, so Radix does not warn about a missing description; an `isLoading` branch the plan did not name; a panel-local `formatAge` that is a deliberate COPY of the module-private helper at `inboxBuckets.ts:190-197` (not exported, and exporting it would touch a list file AC-008 fences off); and the sweep hook ships with no cleanup, because a StrictMode double-effect with an abort would drop the invalidation the first mount fired. Two behavioural defects were caught at diff review, NOT by prediction — the comparator produced NaN on rows with a null `last_message_at` (nullable, T22 Step 0) and re-ordered them arbitrarily, and the sweep's in-flight `cancelled` flag was read but never set. Both fixed before apply; tripwire 0/3 (a caught defect in an unapplied diff is not a prediction miss). **C3a 04944f0** was the single permitted `UnifiedInboxPage.tsx` hunk under AC-008 (+62 lines as committed, later −27 at C3c): `handleAiPanelSelect` is a stable `useCallback` because both moving parts are read through refs — a ref for `close()` and a ref for the current view — so the registration object does not change identity every render; the customers arm reuses the existing deep-link machinery verbatim (`customersDeepLinkConversationIdRef.current = id` then `setCustomersSelection(null)`), never a parallel path; forbidden-token grep over the diff 17 → 17, i.e. no pill/filter/list line touched. Tripwire 0/3 — one line-count slip, ruled bookkeeping. **C3b 215d2b5** came out of browser verify, which showed two row classes in the panel that should not be there: muted rows, and the org's own mailbox. Fix is a `selectNeedsAttention` selector in `src/modules/inbox/utils/needsAttention.ts`. Hidden = the muted HANDLE (`inbox_muted_senders` via `useMutedSenders`, matching `useCustomerThreads.ts:138`) — note this is structurally unlinked-only, since the muted set is keyed by handle and a conversation linked to a person can never match it. Org mailbox = `gmail_connections.email_address`, excluded only when the row is unlinked, so a linked person is never hidden by it. Live (SM, 2026-09-10): 127 open conversations carry the org's own address as `primary_handle`, 23 linked to 23 distinct people, 104 unlinked. **Tripwire 2/3 on C3b** — Q1 predicted mute would be a column on the conversation row and it is a separate table; Q3 predicted the SENT create passes `fromEmail` and it passes `toEmail`. Both are findings about shape rather than execution errors, and both are now recorded (F-036). **C3c de28d33**, ruled by Giorgi after verify: the count bubble is removed entirely, so the registration shape is now `{ title, panel }` — which let the page-side count, `useMutedSenders` and the second `useGmailConnection` come back out of the C3a hunk (−27); in its place an item-count picker of 5/10/25, default 10, persisted to localStorage under `needs_attention_page_size`, rendered as chips using the `PipelinePage.tsx:100-102` selected pairing and aria-labelled. Tripwire 0/3. Two plan amendments shipped in code with `plan.md` deliberately left unedited: **[A6]** `CLAIM_TTL_MS` 10 min and **[A7]** `K_MAX` 25, which makes the SM backfill ≈31 iterations, not the plan's 16 — backlog carries that line as the record. Browser verify passed on each of C3, C3a, C3b and C3c; gate green throughout (tsc 54/54 item-diff 0 new, lint 8/19). C4 (this commit) is docs only: T23 + Arin notes here, F-035 and F-036 in findings.md, eight backlog lines, and the spec.md status/FR annotations below. Next: `reviewer` pass over the full branch diff vs the rules, ff-merge to staging, deploy, Arin heads-up.

Arin notes (2026-09-10, for Giorgi to send — AC-006):
- There is a new AI button on the Inbox only. It is new on the shared page shell, so you may notice the shell changed, but it appears nowhere except the Inbox.
- It opens a "Needs attention" slide-over: your open conversations ranked by the AI, most urgent first, each with a one-line note on what needs doing. Click one and it opens that conversation.
- You choose how many it shows — 5, 10 or 25. It remembers your choice.
- Website-form conversations that carry a message are included in the ranking, not just email and WhatsApp.
- Roughly 127 conversations currently show the business's own email address as the sender, because of a quirk in how mail was brought in. The unlinked ones are hidden from this panel so it stays clean; nothing about your real customer conversations is affected, and fixing the underlying data is on the list.

T22 (2026-09-10, ai-inbox-prioritisation C2 + C2b: edge function + shared module; plan C2, FR-002..FR-009 + FR-018): TWO NEW FILES, no existing file modified — `supabase/functions/_shared/conversationText.ts` (115 lines: MsgRow, stripHtml, sortMessagesLikeUnifiedTimeline, fetchConversationMessages, buildTaggedTranscript, all COPIES from `inbox-ai-thread-summary/index.ts` :33-43 / :15-23 / :59-74 / :323-331 / :529-539, which is NOT modified this cycle) and `supabase/functions/inbox-ai-rank/index.ts` (533 lines). `DENO_NO_PACKAGE_JSON=1 deno check inbox-ai-rank/index.ts` from `supabase/functions/` = 0 errors on both passes; tsc/lint delta 0 (Deno invisible to tsc). DEPLOYED 2026-09-10 **plain** — verify_jwt ON, no config.toml entry (NOT matching the summary function, whose verify_jwt is OFF — corrected 2026-09-19, T30); internal-key callers must therefore also carry `Authorization: Bearer <anon key>` to clear the gateway. Bundle = index.ts + _shared/conversationText.ts + _shared/organizationMembership.ts, single supabase-js specifier `@2.49.4` across all three (two versions in one bundle is the trap; 48/48 repo-wide, 0 others).
Smoke (Giorgi): no auth → 401; internal key + E2E org → {0,0,0}; SM k=5 → {5,5,0}, all five reasons read against their own threads and confirmed thread-specific ([A4] first-scoring proof).
Backfill (SM, k=25): 30 full batches + one of 7, ≈26 s per batch, 0 failed — 757 rows, + the 5 from the k=5 proof = 762, reconciling exactly with the read-back. SC-001: rankable_scored 762 = rankable 762, non_rankable_touched 0. Distribution — priority high (≥70) 23 / medium (40-69) 6 / low 733; category sales 474 / support 52 / admin 26 / other 210. SC-002 top-10 spot-read: all actionable and thread-specific; ONE row scored 85 on a deadline that had already passed → C2b.
C2b (e359e5c, one edit, one line): buildUserContent's facts block gains a first line `- today: <ISO date, UTC>`, derived from the same nowMs the message ages are measured against, so the model can tell whether a mentioned deadline has passed. 532→533 lines, deno check 0, nothing else changed. Deployed; post-deploy SM k=1 → {0,0,0} — nothing stale, so SC-006 (zero AI calls when nothing is stale) is confirmed LIVE, not just by code reading.
Shipped in code, plan.md deliberately NOT edited (amend at C4): **[A6]** stale-claim recovery — ai_scored_at is stamped at claim time BEFORE the rank is written, so a crash or timeout mid-batch would leave a row scored-but-unranked and reading as fresh for REFRESH_DAYS; a row with ai_priority null is tier 1 once its claim exceeds CLAIM_TTL_MS (10 min), tier 0 inside it (a live claim is never double-scored). **[A7]** K_MAX 25, not 50 — sequential OpenAI calls at 50 risk the edge wall-clock limit; K_DEFAULT stays 15, so the backfill loop is k=25 / ≈31 iterations, NOT the plan's k=50 / ≈16. Selection carries a single `.order('last_message_at' desc, nulls last)` so PostgREST db-max-rows truncation is deterministic instead of arbitrary — KNOWN CEILING: above ~1000 rankable rows in one org the OLDEST conversations drop off the selection and never enter the tier-3 refresh (fine at 762; revisit before an org approaches it). Further deviations, all approved at diff: claim/restore guard uses the ai_scored_at string the claim RETURNED (stronger than [A3], which only bars Date round-trips); logDbError logs message+code only, dropping the summary function's details/hint because a CHECK violation would echo ai_reason — model text that can name a customer — into the logs; fetchConversationMessages folds in the separate :462-464 sort and returns {messages, error} rather than throwing, so one row can fail without ending the batch; buildTaggedTranscript truncates a lone over-cap line instead of dropping it to empty; `selected` = rows actually CLAIMED, not shortlisted (a row lost to a concurrent claim counts in neither selected nor failed).
Step 0 (read-only, supabase-ro): FK paths inbox_messages → inbox_conversations = 1 (`inbox_messages_conversation_id_fkey`, ON DELETE CASCADE), reverse direction 0 → the `!inner` embed needs NO FK-name hint (contrast F-025 / invoices_order_id_fkey). All 11 selection columns verified present; last_message_at is NULLABLE (Date.parse → NaN, sorts last, can never satisfy tier 2), enquiry_stage NOT NULL (the plan's "or none" fallback covers empty string, not null).
Function constants: K_DEFAULT 15, K_MAX 25, REFRESH_DAYS 7, CLAIM_TTL_MS 10 min, MAX_MESSAGES_IN_PROMPT 20, TRANSCRIPT_CHAR_CAP 8000, REASON_MAX_CHARS 120 (truncated at store time — no CHECK, so an over-length straggler cannot hard-fail a write). Org resolution adds the membership check the summary function lacks (FR-009): `isUserInOrganization` from _shared/organizationMembership.ts for the given-org path plus an inline count backstop ([A1]: exactly one membership → that org, several → 400); `resolveOrganizationIdForUser` deliberately NOT used — its no-org branch silently picks the first of several memberships.
PII: ai_reason is customer-derived model text. Ids and counts only in docs, migrations, tests and logs; no reason string is reproduced here or in any tracked file.
Tripwire C2 and C2b: 0/3 — zero structural or behavioural misses. Line-count band misses (file 2 predicted 380-430, actual 532; a "111 lines" figure stated before measuring, actual 115) RULED bookkeeping by Giorgi, excluded, and NOT carried forward.
For C4: strengthen the F-0xx wording — the summary function's missing caller-membership check is ONE IMPORT away from a helper that already exists in _shared (`isUserInOrganization`; precedent `gmail-send-first-message/index.ts:3`), so the fix is an import plus a ~6-line guard, not new work; plus the [A6] / [A7] / db-max-rows plan amendments above. Next: C3 (PageShell host + panel + sweep hook) in a fresh session.

T21 (2026-09-10, ai-inbox-prioritisation C1: additive AI rank fields on inbox_conversations; spec FR-023, plan C1): migration 20260910120000_add_inbox_ai_rank_fields.sql COMMITTED 16c7528, PUSHED, then Dashboard-applied 2026-09-10 one statement at a time — four nullable columns (ai_priority, ai_reason, ai_category, ai_scored_at) + two null-tolerant CHECKs (priority range, category vocabulary). Read-backs, all as predicted: views over the table 0 (pre-apply pg_rewrite/pg_depend check); columns like 'ai\_%' 4, all nullable; constraints matching '%ai_%' 2; rows with any ai_* field non-null 0 (nothing rewritten); schema_migrations version row 1 (created_by 'mason:giorgi'). Browser verify on staging: inbox list loads, search returns results, a conversation opens. Tripwire 0/3 (the single line-count miss ruled bookkeeping and excluded — structural/behavioural misses only). FINDING for the drift audit: supabase_migrations.schema_migrations holds ONLY portal-tooling rows (created_by admin@unifynow.digital, latest 2026-08-16); none of Mason's September Dashboard-applied migrations are recorded there, so ABSENCE FROM THAT TABLE PROVES NOTHING about whether a Mason migration ran — F-033's evidence generalised (backlog drift-audit item updated to say so; 20260910120000 is the first hand-inserted Mason row). Next: C2 (edge function + shared module) in a fresh session, tripwire 0/3.
...
CLOSE-OUT (2026-09-03, C11 applied + shell cycle merged at 6edec8c):
DONE — C11 shipped: src/modules/inbox/utils/emailQuotes.ts (new) + ConversationThread.tsx
(12 sites) + the T20 corrections above. Parse-and-mark in the parent, one `:not()` rule in
the frame stylesheet, dataset toggle on the frame root, srcDoc identical across both states.
Two revisions to the approved draft, both from Giorgi's pre-approval questions and both
evidence-led: the abort floor stays 2 (a floor of 10 would abort on "Thank you" and
"Hi <name>", live) with zero-width stripping and a letter-or-digit requirement doing the
work instead; and the hide rule is `html:not([data-mason-quotes="shown"]) [data-mason-quote]
{display:none!important}` rather than a hide/reveal pair using `display:revert` — in the
shown state NO rule of ours applies, which removes the dependency on sanitizeHtml having
stripped every author stylesheet rather than defending it. T604 DONE (merge above).
STILL OPEN, carried forward — none of these were closed by the merge:
(a) F-033 apply-vs-remove (the inbox_messages.message_type migration) — decide INSIDE the
    migration drift audit, not separately; the internal-note button stays hidden until then.
(b) docs/findings.md +14 re-anchor, still outstanding.
(c) Browser verify NEVER FORMALLY RUN for C10b or T19 (a)–(e) — the C10b list above (right-
    panel collapse shrink on conversation c973dd92-d3bb-4d18-b9d0-19552d4ecf93 / message
    6bf0bcce-1f84-40f0-990d-33995decf0a5, first-paint parity, late-image growth, h-scrollbar
    allowance, one-line reply vs the 24px floor) plus C11's own list (default-collapsed
    gmail_quote reply, a quote-first row showing no toggle, Outlook divRplyFwdMsg without
    appendonsend, type=cite and yahoo_quoted rows, a bare-blockquote row unchanged, expanded
    state surviving a prefetch reload, composer typing on the 118,949-char record, viewer
    dialog still unstripped). Merged without them.
(d) Readability pass — UNSTARTED NEW SCOPE, not a leftover.
Tripwire for the C11 apply session: 2/3, heightened caution declared. Miss 1: predicted >21
of the 43 quote-first rows would have 0-1 visible chars; actual 1 — "quote-first" is a byte
offset and does not imply an empty remainder (this is correction (c) above). Miss 2:
predicted a 0/0 lint delta without running gate:lint; actual +1 ERROR
(no-misleading-character-class on the zero-width character class, fixed by matching
codepoints against a Set instead). Process note for the next session: that was the second
unverified lint prediction in three commits, on the commit immediately after C10b shipped
over an unread lint count. gate:lint is cheap — run it before stating any lint delta, or say
the prediction is unverified.
