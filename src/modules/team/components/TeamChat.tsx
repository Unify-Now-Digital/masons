    ...
    return parts.map((part, i) => {
      if (part.startsWith('ORD-') || part.startsWith('#ORD-')) {
        ...
        return (
          <button 
            key={i} 
            className="inline-flex items-center gap-1 mx-1 px-1.5 py-0.5 bg-slate-900 text-white rounded text-[10px] font-black uppercase tracking-tighter hover:bg-blue-600 transition-colors shadow-sm"
          >
      ...
      if (part.startsWith('@')) {
        return (
          <button 
            key={i} 
            className="inline-flex items-center gap-1 mx-1 px-1.5 py-0.5 bg-blue-100 text-blue-700 rounded text-[10px] font-black uppercase tracking-tighter hover:bg-blue-200 transition-colors"
          >
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-8 custom-scrollbar">
          <section>
            <div className="flex justify-between items-center px-3 mb-4">
              ...
              <button className="text-blue-600"><Plus className="w-3.5 h-3.5" /></button>
            </div>
              ...
              {CHANNELS.map(ch => (
                ...
                >
                  <div className="flex items-center gap-3">
                    <Hash className={`w-4 h-4 ${activeChannel === ch.id ? 'text-white/50' : 'text-slate-300'}`} />
                  ...
                  {ch.count > 0 && (
                    <span className={`w-5 h-5 flex items-center justify-center rounded-lg text-[10px] font-black ${
                      activeChannel === ch.id ? 'bg-white/20 text-white' : 'bg-orange-500 text-white'
                    }`}>
                      {ch.count}
                    </span>
                  )}
          ...
          <section>
            <div className="flex justify-between items-center px-3 mb-4">
              <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Team Relay</p>
            </div>
            <div className="space-y-1">
      ...
      <div className="flex-1 flex flex-col relative">
        ...
        <div className="px-4 xl:px-8 py-4 xl:py-5 border-b flex justify-between items-center bg-white h-[65px] xl:h-[85px] shrink-0">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h3 className="text-lg font-black text-slate-900 capitalize tracking-tight">{activeChannel}</h3>
              <p className="text-[10px] font-bold text-slate-400 uppercase mt-0.5">Topic: Coordinating floor production and fixes</p>
            </div>
          </div>
          <div className="flex items-center gap-4">
             <div className="flex -space-x-3">
                <div className="w-9 h-9 rounded-full bg-blue-100 border-4 border-white flex items-center justify-center text-[10px] font-bold text-blue-600">MM</div>
                <div className="w-9 h-9 rounded-full bg-green-100 border-4 border-white flex items-center justify-center text-[10px] font-bold text-green-600">SA</div>
                <div className="w-9 h-9 rounded-full bg-slate-100 border-4 border-white flex items-center justify-center text-[10px] font-bold text-slate-400">+2</div>
             </div>
             <div className="h-6 w-px bg-slate-200" />
             <button className="p-2 text-slate-400 hover:text-slate-900 transition-colors"><Info className="w-5 h-5" /></button>
          </div>
        </div>
          ...
          {chatHistory.map((msg) => (
            <div key={msg.id} className="flex gap-5 group animate-in slide-in-from-bottom-2 duration-300">
               <div className={`w-12 h-12 rounded-2xl flex items-center justify-center shrink-0 font-black text-sm shadow-sm border transition-all ${
                 msg.type === 'ai' ? 'bg-blue-600 text-white border-blue-500' : 'bg-white text-slate-400 border-slate-100 group-hover:border-blue-200'
               }`}>
                 {msg.avatar}
               </div>
               <div className="space-y-1.5 flex-1 min-w-0">
                 <div className="flex items-center gap-3">
                   <span className="text-sm font-black text-slate-900">{msg.user}</span>
                   {msg.type === 'ai' && (
                     <span className="bg-blue-50 text-blue-600 text-[8px] font-black px-1.5 py-0.5 rounded uppercase flex items-center gap-1">
                 ...
                 <div className={`px-5 py-4 rounded-3xl rounded-tl-none shadow-sm border text-[13px] leading-relaxed max-w-2xl font-medium ${
                   msg.type === 'ai' ? 'bg-blue-50 border-blue-100 text-blue-900' : 'bg-white border-slate-200 text-slate-700'
                 }`}>
        ...
        {/* Mentions Dropdown */}
        {mentionMenu.isOpen && (
          <div className="absolute bottom-32 left-8 w-80 bg-white rounded-3xl shadow-[0_20px_50px_rgba(0,0,0,0.15)] border border-slate-200 overflow-hidden z-[100] animate-in slide-in-from-bottom-4 duration-200">
            <div className="p-4 bg-slate-50 border-b flex items-center gap-2">
               {mentionMenu.type === 'order' ? <ShoppingCart className="w-3.5 h-3.5 text-blue-500" /> : <User className="w-3.5 h-3.5 text-blue-500" />}
            ...
            <div className="max-h-60 overflow-y-auto">
              {filteredMentions.length > 0 ? filteredMentions.map((item: any) => (
                <button 
                  ...
                  className="w-full flex items-center gap-4 px-5 py-3.5 hover:bg-slate-50 text-left transition-all border-b border-slate-50 last:border-0"
                >
                  ...
                  <div>
                    <p className="text-sm font-black text-slate-900">{mentionMenu.type === 'order' ? item.customerName : item.name}</p>
                ...
                <div className="p-6 text-center text-slate-400 text-[10px] font-black uppercase">No matches found</div>
              )}
            </div>
          </div>
        )}
        ...
        <div className="p-4 xl:p-8 bg-white border-t shrink-0">
          <div className="bg-slate-50 rounded-[2rem] border border-slate-200 p-3 shadow-inner group focus-within:bg-white focus-within:ring-4 focus-within:ring-blue-500/5 transition-all">
             ...
             />
             <div className="flex justify-between items-center px-4 pb-2 pt-2 border-t border-slate-200/50">
          ...
          <p className="text-[9px] font-bold text-slate-300 uppercase tracking-widest mt-4 text-center">
...
const DMItem = ({ name, status, isMe }: { name: string, status: 'online' | 'away' | 'offline', isMe?: boolean }) => (
  <button className="w-full flex items-center justify-between px-4 py-3 rounded-2xl text-sm font-bold text-slate-600 hover:bg-white transition-all group">
    <div className="flex items-center gap-3">
      <div className="relative">
        <div className="w-9 h-9 rounded-xl bg-slate-100 flex items-center justify-center text-[11px] font-black text-slate-400 group-hover:text-blue-600 transition-colors">
          {name.split(' ').map(n => n[0]).join('')}
        </div>
        <div className={`absolute -bottom-1 -right-1 w-3.5 h-3.5 rounded-full border-4 border-slate-50 ${
          status === 'online' ? 'bg-emerald-500' : status === 'away' ? 'bg-amber-400' : 'bg-slate-300'
        }`} />
      </div>
      <div>
        <p className="tracking-tight text-slate-900">{name} {isMe && <span className="text-[10px] text-slate-400 opacity-50 font-medium">(you)</span>}</p>
        <p className="text-[9px] font-black uppercase text-slate-400 opacity-0 group-hover:opacity-100 transition-opacity tracking-tighter">View Profile</p>
      </div>
    </div>
...
const ChatTool = ({ icon, onClick }: { icon: React.ReactNode, onClick?: () => void }) => (
  <button 
    onClick={onClick}
    className="p-2.5 hover:bg-slate-200 rounded-xl text-slate-400 hover:text-slate-900 transition-all shadow-sm bg-white border border-slate-100 active:scale-90"
  >
    {icon}
  </button>
);
